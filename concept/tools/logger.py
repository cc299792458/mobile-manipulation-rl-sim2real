    ...
    def writekvs(self, kvs):
        ...
        for (key, val) in sorted(kvs.items()):
            ...
            if hasattr(val, "__float__"):
                ...
                valstr = str(val)
            if key + ":min" in kvs:
                valstr = f"{kvs[key + ':min']:-9.4g} | {val - kvs[key + ':std']:-9.4g} | {val:-9.4g} | {val + kvs[key + ':std']:-9.4g} | {kvs[key + ':max']:-9.4g}"
...
def logkvs_min_avg_max(d, prefix=""):
    """
    log a dictionary of key-value pairs, where the value can be a tensor
    if the value is a tensor, log the min, avg, and max
    """
    import torch
    to_log = dict()
    if prefix != "":
    ...
    for k, v in d.items():
        if isinstance(v, torch.Tensor):
            if v.numel() == 0:
                ...
                to_log[f"{prefix}{k}"] = v_mean
                to_log[f"{prefix}{k}:min"] = v.min().item()
                to_log[f"{prefix}{k}:std"] = v_std
                to_log[f"{prefix}{k}:max"] = v.max().item()
        else:
            to_log[f"{prefix}{k}"] = v
    logkvs(to_log)
...
def log(*args):
    ...
    """
    time_started = get_current().time_started
    time_now = datetime.datetime.now()
    tplus = time_now - time_started
    tplus_hours = tplus.total_seconds() / 3600
    get_current().log(
        (
            f"[T:{time_now}] ==== [T+{tplus_hours:.3f}] ==== [T0:{time_started}]\n"
            f"[dir]{get_dir()} [cmd] python {' '.join(sys.argv)}\n[log]"
        ), *args
    )
...
def configure(
    ...
    from concept.tools import dist_utils as comm
    if comm.get_rank() == 0:
        log_suffix = ""
    else:
